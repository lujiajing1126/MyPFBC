...
class Form extends Base {
	...
	public function render($extrajs = array(),$extracss = array(), $returnHTML = false) {
		...
		$this->renderCSS($extracss);
		$this->view->render();
		$this->renderJS($extrajs);
	...
	protected function renderCSS($extracss) {
		$this->renderCSSFiles($extracss);
	...
	protected function renderCSSFiles($extracss) {
		$urls = array();
		...
		if (!empty($extracss)) {
			foreach ($extracss as $cssurl)  {
				echo '<link type="text/css" rel="stylesheet" href="', $cssurl, '"/>';;
			}
		}
	...
	protected function renderJS($extrajs) {
		$this->renderJSFiles($extrajs);	
...
JS;
	...
	protected function renderJSFiles($extrajs) {
		$urls = array();
